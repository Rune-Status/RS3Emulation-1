...
/**
 * An {@link PacketDecoder} that decodes the streams for player chatting.
 *
 * @author Thomas Le Godais <thomaslegodais@live.com>
 *
 */
public class ChatDecoder implements PacketDecoder<ChatContext> {

	/* (non-Javadoc)
	 * @see net.ieldor.network.packet.PacketDecoder#decode(net.ieldor.io.PacketReader)
	 */
	@Override
	public ChatContext decode(PacketReader packet) {
		int colour = packet.get();
		int effects = packet.get();
		int numChars = packet.get();
		int size = packet.getLength() - 2;

		final String text = decryptPlayerChat(packet, numChars);
		return new ChatContext(colour, effects, size, text);
	}
	...
	public static int[] MASKS = { 0, 1024, 2048, 3072, 4096, 5120,
	...
	public static byte[] BITS = { 22, 22, 22, 22, 22, 22, 21, 22, 22,
	...
	public static int encryptPlayerChat(String str, byte[] dest) {
		if (str.length() > 80)
		...
		dest[0] = (byte) len;
		for (int i = 0; i < len; i++) {
			int chr = str.charAt(i) & 0xFF;
			int mask = MASKS[chr];
			int size = BITS[chr];

			off = bitPos >> 3;
			int remainder = bitPos & 0x7;
			key &= (-remainder >> 31);
			bitPos += size;
			int end = (((remainder + size) - 1) >> 3) + off;

			remainder += 24;
			dest[off] = (byte) (key |= (mask >>> remainder));

			for (int j = 0; j < 4 && off < end; j++) {
				remainder -= 8;
				dest[++off] = (byte) (key = mask >>> remainder);
			}
		}
