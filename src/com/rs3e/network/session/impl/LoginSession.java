...
import com.rs3e.network.protocol.codec.login.LoginPayload;
...
/**
 * 
 * RS3Emulation LoginSession.java Mar 11, 2014
 * 
...
public class LoginSession extends Session {
	...
	private void decodeLobbyLogin(ByteBuf buffer) {
		...
		xtea.decipher(xteaBlock, 0, xteaBlock.length);
		...
		if (GeneralUtils.invalidAccountName(username)) {
			// session.getLoginPackets().sendClientPacket(3);//Invalid username
			// or password
		...
		if (!SerializableFilesManager.containsPlayer(username)) {
			...
			if (player == null) {
				// session.getLoginPackets().sendClientPacket(20);//Invalid
				// login server
				channel.write(new LoginResponse(
						LoginResponse.INVALID_LOGIN_SERVER));
				return;
			}
			/*
			 * if (!SerializableFilesManager.createBackup(username)) {
			 * //session.getLoginPackets().sendClientPacket(20);//Invalid login
			 * server //return; }
			 */
